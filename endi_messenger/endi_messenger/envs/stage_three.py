...
class StageThree(MessengerEnv):
    ...
    def _move_goal1(self):
        action =  random.randint(0, 10)
        action = action if action < 4 else 4
        if action == config.ACTIONS.stay:
            return
        
        elif action == config.ACTIONS.up:
            if self.goal1.position.y <= 0:
                return
            else:
                tmp_position = Position(
                    y = self.goal1.position.y - 1,
                    x = self.goal1.position.x
                )
                self.tmp = Sprite(
                    name=self.goal1.name,
                    id=self.goal1.id,
                    position=tmp_position
                )
                for sprite in (self.enemy, self.message1, self.message2, self.goal2):
                    if self._overlap(self.tmp, sprite):
                        return
                new_position = Position(
                    y = self.goal1.position.y - 1,
                    x = self.goal1.position.x
            ...
            if self.goal1.position.y >= config.STATE_HEIGHT - 1:
                return
            else:
                tmp_position = Position(
                    y = self.goal1.position.y + 1,
            ...
            if self.goal1.position.x <= 0:
                return
            else:
                tmp_position = Position(
                    y = self.goal1.position.y,
                    x = self.goal1.position.x - 1
                )
                self.tmp = Sprite(
                    name=self.goal1.name,
                    id=self.goal1.id,
            ...
            if self.goal1.position.x >= config.STATE_WIDTH - 1:
                return
            else:
                tmp_position = Position(
                    y = self.goal1.position.y,
    ...
    def _move_goal2(self):
        ...
        if action == config.ACTIONS.stay:
            ...
            if self.goal2.position.y <= 0:
                return
            else:
                tmp_position = Position(
                    y = self.goal2.position.y - 1,
                    x = self.goal2.position.x
                )
                self.tmp = Sprite(
                    name=self.goal2.name,
                    id=self.goal2.id,
                    position=tmp_position
                )
                for sprite in (self.enemy, self.message1, self.message2, self.goal1):
                    if self._overlap(self.tmp, sprite):
                        return
                new_position = Position(
                    y = self.goal2.position.y - 1,
            ...
            if self.goal2.position.y >= config.STATE_HEIGHT - 1:
                return
            else:
                tmp_position = Position(
                    y = self.goal2.position.y + 1,
            ...
            if self.goal2.position.x <= 0:
                return
            else:
                tmp_position = Position(
                    y = self.goal2.position.y,
                    x = self.goal2.position.x - 1
                )
                self.tmp = Sprite(
                    name=self.goal2.name,
                    id=self.goal2.id,
            ...
            if self.goal2.position.x >= config.STATE_WIDTH - 1:
                return
            else:
                tmp_position = Position(
                    y = self.goal2.position.y,
    ...
    def _move_message1(self):
        ...
        if action == config.ACTIONS.stay:
            ...
            if self.message1.position.y <= 0:
                return
            else:
                tmp_position = Position(
                    y = self.message1.position.y - 1,
                    x = self.message1.position.x
                )
                self.tmp = Sprite(
                    name=self.message1.name,
                    id=self.message1.id,
                    position=tmp_position
                )
                for sprite in (self.enemy, self.message2, self.goal1, self.goal2):
                    if self._overlap(self.tmp, sprite):
                        return
                new_position = Position(
                    y = self.message1.position.y - 1,
            ...
            if self.message1.position.y >= config.STATE_HEIGHT - 1:
                return
            else:
                tmp_position = Position(
                    y = self.message1.position.y + 1,
            ...
            if self.message1.position.x <= 0:
                return
            else:
                tmp_position = Position(
                    y = self.message1.position.y,
                    x = self.message1.position.x - 1
                )
                self.tmp = Sprite(
                    name=self.message1.name,
                    id=self.message1.id,
            ...
            if self.message1.position.x >= config.STATE_WIDTH - 1:
                return
            else:
                tmp_position = Position(
                    y = self.message1.position.y,
        ...
        self.message1 = Sprite(
                name=self.message1.name,
                id=self.message1.id,
                position=new_position
            )

    def _move_message2(self):
        action =  random.randint(0, 10)
        action = action if action < 4 else 4
        if action == config.ACTIONS.stay:
            return
            ...
            if self.message2.position.y <= 0:
                return
            else:
                tmp_position = Position(
                    y = self.message2.position.y - 1,
                    x = self.message2.position.x
                )
                self.tmp = Sprite(
                    name=self.message2.name,
                    id=self.message2.id,
                    position=tmp_position
                )
                for sprite in (self.enemy, self.message1, self.goal1, self.goal2):
                    if self._overlap(self.tmp, sprite):
                        return
                new_position = Position(
                    y = self.message2.position.y - 1,
            ...
            if self.message2.position.y >= config.STATE_HEIGHT - 1:
                return
            else:
                tmp_position = Position(
                    y = self.message2.position.y + 1,
            ...
            if self.message2.position.x <= 0:
                return
            else:
                tmp_position = Position(
                    y = self.message2.position.y,
                    x = self.message2.position.x - 1
                )
                self.tmp = Sprite(
                    name=self.message2.name,
                    id=self.message2.id,
            ...
            if self.message2.position.x >= config.STATE_WIDTH - 1:
                return
            else:
                tmp_position = Position(
                    y = self.message2.position.y,
    ...
    def _move_enemy(self):
        ...
        if action == config.ACTIONS.stay:
            ...
            if self.enemy.position.y <= 0:
                ...
                for sprite in (self.message1, self.message2, self.goal1, self.goal2):
                    if self._overlap(self.tmp, sprite):
                        return
                new_position = Position(
                    y = self.enemy.position.y - 1,
            ...
            if self.enemy.position.y >= config.STATE_HEIGHT - 1:
                return
            else:
                tmp_position = Position(
                    y = self.enemy.position.y + 1,
            ...
            if self.enemy.position.x <= 0:
                ...
                tmp_position = Position(
                    y = self.enemy.position.y,
                    x = self.enemy.position.x - 1
                )
                self.tmp = Sprite(
                    name=self.enemy.name,
                    id=self.enemy.id,
            ...
            if self.enemy.position.x >= config.STATE_WIDTH - 1:
                return
            else:
                tmp_position = Position(
                    y = self.enemy.position.y,
    ...
    def step(self, actions):
        ...
        if self._overlap(self.avatar1, self.message1) or self._overlap(self.avatar1, self.message2):
            if self.avatar1.name == config.NO_MESSAGE.name:
                self.avatar1 = Sprite(
                    name=config.WITH_MESSAGE.name,
                    id=config.WITH_MESSAGE.id,
                    position=self.avatar1.position
                )
                self.a1_nomsg = False
                tmp_reward += bonus
            
        if self._overlap(self.avatar2, self.message1) or self._overlap(self.avatar2, self.message2):
            if self.avatar2.name == config.NO_MESSAGE.name:
                self.avatar2 = Sprite(
                    name=config.WITH_MESSAGE.name,
                    id=config.WITH_MESSAGE.id,
                    position=self.avatar2.position
                )
                tmp_reward += bonus

        if self._overlap(self.avatar1, self.goal1) or self._overlap(self.avatar1, self.goal2):
            if self.avatar1.name == config.WITH_MESSAGE.name:
                tmp_reward += bonus
                if self._overlap(self.avatar1, self.goal1):
                    self.a1_g1 = True
                else:
                    self.a1_g2 = True
            elif self.avatar1.name == config.NO_MESSAGE.name:
                return obs, -1.0, True, None
            else:
                raise Exception("Unknown avatar name")
            
        if self._overlap(self.avatar2, self.goal1) or self._overlap(self.avatar2, self.goal2):
            if self.avatar2.name == config.WITH_MESSAGE.name:
                tmp_reward += bonus
                if self._overlap(self.avatar2, self.goal1):
                    self.a2_g1 = True
                else:
                    self.a2_g2 = True
            elif self.avatar2.name == config.NO_MESSAGE.name:
                return obs, -1.0, True, None
            else:
                raise Exception("Unknown avatar name")

        if (self.a1_g1 and self.a2_g2) or (self.a1_g2 and self.a2_g1):
            return obs, 1.0, True, None
        
        return obs, 0.0 + tmp_reward, False, None
