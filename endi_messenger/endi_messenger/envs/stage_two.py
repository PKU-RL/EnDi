...
class StageTwo(MessengerEnv):
    ...
    def step(self, actions):
        ...
        if (self._overlap(self.avatar1, self.message1) and self._overlap(self.avatar2, self.message2)) or (self._overlap(self.avatar1, self.message2) and self._overlap(self.avatar2, self.message1)):
            if self.avatar1.name == config.WITH_MESSAGE.name and self.avatar2.name == config.WITH_MESSAGE.name:
                return obs, -1.0, True, None
            elif self.avatar1.name == config.NO_MESSAGE.name or self.avatar2.name == config.NO_MESSAGE.name:
                return obs, 1.0, True, None
            else:
                raise Exception("Unknown avatar name")
            
        if (self._overlap(self.avatar1, self.goal1) and self._overlap(self.avatar2, self.goal2)) or (self._overlap(self.avatar1, self.goal2) and self._overlap(self.avatar2, self.goal1)):
            if self.avatar1.name == config.WITH_MESSAGE.name or self.avatar2.name == config.WITH_MESSAGE.name:
                return obs, 1.0, True, None
            elif self.avatar1.name == config.NO_MESSAGE.name and self.avatar2.name == config.NO_MESSAGE.name:
                return obs, -1.0, True, None
            else:
                raise Exception("Unknown avatar name")
