'''
Script that allows users to play Multi-Agent Messenger in the terminal.
'''
...
if __name__ == "__main__":
    ...
    parser.add_argument("--stage", type=str, required=True, help='environment id for human play')
    ...
    while keep_playing.lower() not in ["no", "n"]:
        ...
        while not done:
            actions = action.lower().split(' ')
            if actions[0] in action_map and actions[1] in action_map:
                obs, reward, done, info = env.step([action_map[actions[0]],action_map[actions[1]]])
                eps_steps += 1
                eps_reward += reward
                clear_terminal()
                print_instructions()
